...
- 아카이브 화면에서 작가/태그를 탭하면 해당 작가·태그의 작품 목록 화면(`artist/{artistId}`, `tag/{tagId}`, 공용 `ArtworkListScreen`)으로 이동.
- 전시 목록(홈 최근 전시/아카이브 전시 탭/위시리스트)을 그리드로 표시, 한 줄에 표시할 열 수(1~4)를 설정 화면에서 조절.
- 달력: 그날 방문한 전시에 포스터가 있으면 셀 전체를 포스터로 채우고 날짜 대신 그날의 관람 건수를 표시, 포스터가 없으면 날짜 숫자만 표시. 위시리스트 전시 기간에 해당하는 날짜에는 셀 우하단에 전시별 고정 색점을 표시(팔레트 순환), 달력 하단에 위시리스트 범례와 그 달의 관람 전시 목록을 함께 보여준다. 날짜를 탭하면 그날의 관람/위시리스트 일정을 보여주는 `DayDetailDialog`가 뜨고, 다이얼로그 안의 작은 + 버튼으로 그 날짜를 관람일로 미리 채운 전시 등록 또는 작품 등록으로 바로 진입할 수 있다.
