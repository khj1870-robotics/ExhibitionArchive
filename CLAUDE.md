...
- 작품 태그 입력이 전시 태그와 동일하게 전면 연결됨(`ArtworkCreateScreen`/`ArtworkEditScreen`/`ArtworkQuickAddDialog`), 모든 태그 입력란에 다다이즘/인상주의/키네틱아트/바로크 등 예시 태그를 사조/양식/장르·기법 대분류로 묶어 `AssistChip`으로 제시해 탭 한 번으로 추가 가능(`TagInputField` + `TAG_PRESET_CATEGORIES`). 작품 상세정보창에도 태그를 표시.
- 날짜 입력칸(관람일/전시 시작일·종료일)은 모두 직접 타이핑 대신 탭하면 Material3 캘린더가 뜨는 `DateField`로 통일.
- 작품 추가 화면에 "완료" 버튼 추가 — 입력 중인 내용이 있으면 저장 후, 없으면 바로 저장 없이 나간다("저장하고 계속 추가"와 별도).
- 관람 모드: 촬영/사진 첨부/메모 추가/음성 저장이 완료되면 화면 상단에 작게 떴다가 자동으로 사라지는 팝업으로 알려준다(에러 메시지는 기존처럼 인라인 유지). 빠른 메모 추가 버튼을 누르면 키보드가 자동으로 내려간다.
