...
사이트 한 곳의 수집이 실패해도 나머지는 계속 갱신한다. 정적 요청으로 목록을 읽지 못하는 사이트만 Chromium 렌더링을 보조 수단으로 사용한다. 이전 정상 데이터가 있으면 실패한 사이트의 목록은 마지막 정상 버전을 유지하고 수집 상태를 `stale`로 표시한다. 사용자 검색어, 관람 기록, 감상과 사진은 수집기로 전송하지 않는다.
